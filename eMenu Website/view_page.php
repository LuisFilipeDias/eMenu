...
for ($i = 1; $i <= 10; $i++) {
	...
	$sql = mysql_query("SELECT * FROM members WHERE id='$i'"); 
	while($row = mysql_fetch_array($sql)){
		$country[$i] = $row["country"];
		$state[$i] = $row["state"];
		$city[$i] = $row["City"];
		$accounttype[$i] = $row["accounttype"];	
		$bio[$i] = $row["bio"];	
	}
}
...
<table width="768" cellpadding="3" cellspacing="3" style="line-height:1.5em;">
  <tr>
    ...
    <td width="423" valign="top">
      Prato A: <?php echo "$country[9]"; ?> <br />
      Prato B: <?php echo "$state[9]"; ?><br />
      Prato C: <?php echo "$city[9]"; ?><br />
      Prato A: <?php echo "$country[10]"; ?> <br />
      Prato B: <?php echo "$state[10]"; ?><br />
      Prato C: <?php echo "$city[10]"; ?><br />
    </td>
  </tr>
  <tr>
